...
public class GroupManager extends BaseActivity {
    ...
    private void initView() {
        ...
        // 取消管理员，隐身人，监控人的按钮设置监听，
        @SuppressLint("UseSparseArrays")
        Map<Integer, Integer> cancelRoleIdMap = new HashMap<>();
        cancelRoleIdMap.put(R.id.cancel_manager_rl, RoomMember.ROLE_MANAGER);
        cancelRoleIdMap.put(R.id.cancel_invisible_rl, RoomMember.ROLE_INVISIBLE);
        cancelRoleIdMap.put(R.id.cancel_guardian_rl, RoomMember.ROLE_GUARDIAN);
        for (Integer id : cancelRoleIdMap.keySet()) {
            findViewById(id).setOnClickListener(v -> {
                CancelManagerActivity.start(this, mRoomId, mRoomJid, cancelRoleIdMap.get(id));
            });
        }
