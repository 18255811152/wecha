...
public class ChatMessageDao {
     ...
     * 更改一条消息的发送 次数
     * */
    public boolean updateChatMessageSendCount(String ownerId, String friendId, String packetId, int count) {
        try {
            Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
            UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();
            ChatMessage message = findMsgById(ownerId, friendId, packetId);
            if (message != null) {
                builder.updateColumnValue("reSendCount", count);
    ...
    /**
     * 更新filesize，其实就是更新红包是否已经领取过了
     * 因为把红包的领取状态之存在了fileSize里面，所以需要增加一个更新的方法
     * 防止第二次输入同样的口令是还会打开这个红包
     *
     * @param ownerId
     * @param friendId
     * @param packetId
     * @param fileSize
     */
    public void updateMessageFilesize(String ownerId, String friendId, String packetId, int fileSize) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return;
        }
        ...
        try {
            builder.updateColumnValue("fileSize", fileSize);
            builder.where().eq("packetId", packetId);
            dao.update(builder.prepare());
        } catch (SQLException e) {
            e.printStackTrace();
